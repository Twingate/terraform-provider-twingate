	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			attr.OidcURL: schema.StringAttribute{
				Computed:    true,
				Description: `The IAM Identity Provider URL (return only if type "oidc"). Example: https://tenant.twingate.com/oidc/v2`,
			},
			attr.OidcPrefix: schema.StringAttribute{
				Computed:    true,
				Description: `The IAM Identity Provider prefix (return only if type "oidc"). Example: tenant.twingate.com/oidc/v2`,
			},
		},
	}
}
...
func (d *syncToS3) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if data.Type.ValueString() == model.SyncToS3TypeOIDC {
		url, err := d.client.ReadSyncToS3OidcURL(ctx)
		if err != nil {
			addErr(&resp.Diagnostics, err, TwingateSyncToS3)
