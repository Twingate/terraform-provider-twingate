...
func init() {
	sdk.AddTestSweepers(resource.TwingateX509CertificateAuthority, &sdk.Sweeper{
		Name: resource.TwingateX509CertificateAuthority,
		F: newTestSweeper(resource.TwingateX509CertificateAuthority,
			func(providerClient *client.Client, ctx context.Context) ([]Resource, error) {
				resources, err := providerClient.ReadX509CertificateAuthorities(ctx)
			...
			func(providerClient *client.Client, ctx context.Context, id string) error {
				return providerClient.DeleteX509CertificateAuthority(ctx, id)
			},
		),
	})
}
