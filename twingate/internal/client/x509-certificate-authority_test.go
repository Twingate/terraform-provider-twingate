...
func TestReadX509CertificateAuthorities(t *testing.T) {
	cases := []struct {
		name         string
		responseBody string
		...
		{
			name: "only X509 CAs - all returned",
			responseBody: `{"data":{"certificateAuthorities":{"pageInfo":{"endCursor":"","hasNextPage":false},"edges":[
				{"node":{"__typename":"X509CertificateAuthority","id":"x509-ca-1","name":"x509-ca-1","fingerprint":"fp1"}},
				{"node":{"__typename":"X509CertificateAuthority","id":"x509-ca-2","name":"x509-ca-2","fingerprint":"fp2"}}
			]}}}`,
			expected: []*model.CertificateAuthority{
				{ID: "x509-ca-1", Name: "x509-ca-1", Fingerprint: "fp1"},
				{ID: "x509-ca-2", Name: "x509-ca-2", Fingerprint: "fp2"},
			},
		},
		{
			name: "mixed CA types - only X509 CAs returned",
			responseBody: `{"data":{"certificateAuthorities":{"pageInfo":{"endCursor":"","hasNextPage":false},"edges":[
			...
			expected: []*model.CertificateAuthority{
				{ID: "x509-ca-1", Name: "x509-ca", Fingerprint: "fp2"},
			},
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			authorities, err := client.ReadX509CertificateAuthorities(context.Background())
