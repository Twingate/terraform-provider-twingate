...
func (client *Client) ReadSecurityPolicies(ctx context.Context, filter *StringFilter) ([]*model.SecurityPolicy, error) {
	opr := resourceSecurityPolicy.read()

	variables := newVars(
		gqlNullable(query.NewSecurityPolicyFilterField(filter.ToQuery()), "filter"),
