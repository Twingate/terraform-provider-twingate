...
func main() {
	...
	//}
	processItem("/Users/vmanilo/workspace/twingate/terraform-provider-twingate/examples/resources/twingate_group/resource.tf")
}
...
func processFile(fn string, mode os.FileMode) {
	...
	//	return
	//}
	log.Printf("Has changes: %s", string(newSrc))
}
...
func cleanBody(body *hclwrite.Body, inBlocks []string) {
	...
	for name, attr := range attrs {
		fmt.Printf("name: %s, attr: %s\n", name, string(attr.Expr().BuildTokens(nil).Bytes()))
	...
	for _, block := range blocks {
		...
		fmt.Printf("block type: %v, labels: %v\n", block.Type(), block.Labels())
