...
echo "Running tests:"
"$(go env GOPATH)"/bin/gotestsum --format standard-verbose --junitfile "${TEST_RESULTS}"/test-results.xml -- -coverpkg=./twingate/... -coverprofile="${TEST_RESULTS}"/coverage.out.tmp "${PACKAGE_NAMES}"
echo
