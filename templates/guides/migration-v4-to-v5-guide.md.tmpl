...
## Migrating SSH Resource Username

The `username` attribute has been removed from the `twingate_ssh_resource` resource. It only ever fed the Gateway's configuration file, which listed each SSH upstream along with the user to connect as. The Gateway now takes the username from the runtime connection, so with `twingate_gateway_config` gone the attribute has nothing left to configure.
...
From v5.0.0 and onward, the `username` attribute must be removed:
...
A configuration that still sets `username` fails with `Unsupported argument`. No state change is needed, because the attribute never reached the Twingate API.

This is a different setting from `ssh.gateway.username` in the Gateway's configuration file, which names the operating system user the Gateway process runs as. That one is still required.
